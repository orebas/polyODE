...
TEST(RationalFunctionTest, ConstructorsAndNormalization) {
    Polynomial<double> Px(x); // x
    Polynomial<double> Py(y); // y
    // Explicitly construct from Monomial for scalar
    Polynomial<double> P1{ Monomial<double>(1.0) };
    Polynomial<double> P0;
    Monomial<double> My(1.0, y);

    // Default: 0/1
    RationalFunction<double> rf_default; // Use default constructor for 0/1
    EXPECT_POLY_EQ(rf_default.numerator, P0);
    EXPECT_POLY_EQ(rf_default.denominator, P1);

    // From Polynomials: x/y (Use {} initializer)
    RationalFunction<double> rf_xy{ Px, Py };
    ...
    RationalFunction<double> rf_5_over_1{ Polynomial<double>(Monomial<double>(5.0)) };
    ...
    RationalFunction<double> rf_0_over_y{ P0, Py };
    EXPECT_POLY_EQ(rf_0_over_y.numerator, P0);
    EXPECT_POLY_EQ(rf_0_over_y.denominator, P1);

    // Normalization: (2x)/(2y) -> x/y (Use {} initializer)
    Polynomial<double> P2x = Polynomial<double>(Monomial<double>(2.0)) * Px;
    Polynomial<double> P2y = Polynomial<double>(Monomial<double>(2.0)) * Py;
    RationalFunction<double> rf_2x_over_2y{ P2x, P2y };
    ...
    Polynomial<double> Px_minus_x = Px - Px;
    EXPECT_THROW(RationalFunction<double>(Px, Px_minus_x), std::invalid_argument);
}

TEST(RationalFunctionTest, Arithmetic) {
    // Use {} initializer
    RationalFunction<double> A{ x };                                                                // x/1
    RationalFunction<double> B{ y };                                                                // y/1
    RationalFunction<double> C{ Polynomial<double>(Monomial<double>(1.0)), Polynomial<double>(x) }; // 1/x
    RationalFunction<double> D{ Polynomial<double>(y), Polynomial<double>(x) };                     // y/x

    // Addition: x + y = (x+y)/1
    RationalFunction<double> res_add = A + B;
    EXPECT_RF_EQ(res_add, RationalFunction<double>{ Polynomial<double>(x) + Polynomial<double>(y) });

    // Addition: x + 1/x = (x^2 + 1)/x
    RationalFunction<double> res_add2 = A + C;
    Polynomial<double> num_add2 =
      Polynomial<double>(Monomial<double>(1.0, x, 2)) + Polynomial<double>(Monomial<double>(1.0));
    Polynomial<double> den_add2 = Polynomial<double>(x);
    EXPECT_RF_EQ(res_add2, RationalFunction<double>{ num_add2, den_add2 });

    // Subtraction: y - x = (y-x)/1
    RationalFunction<double> res_sub = B - A;
    EXPECT_RF_EQ(res_sub, RationalFunction<double>{ Polynomial<double>(y) - Polynomial<double>(x) });

    // Subtraction: y/x - 1/x = (y-1)/x
    RationalFunction<double> res_sub2 = D - C;
    Polynomial<double> num_sub2 = Polynomial<double>(y) - Polynomial<double>(Monomial<double>(1.0));
    Polynomial<double> den_sub2 = Polynomial<double>(x);
    EXPECT_RF_EQ(res_sub2, RationalFunction<double>{ num_sub2, den_sub2 });

    // Multiplication: x * y = xy/1
    RationalFunction<double> res_mul = A * B;
    ...
    RationalFunction<double> res_mul2 = A * D;
    // Without GCD: xy/x
    Polynomial<double> num_mul2 = Polynomial<double>(Monomial<double>(1.0, { { x, 1 }, { y, 1 } }));
    Polynomial<double> den_mul2 = Polynomial<double>(x);
    ...
    RationalFunction<double> res_div = A / B;
    EXPECT_RF_EQ(res_div, RationalFunction<double>{ Polynomial<double>(x), Polynomial<double>(y) });

    // Division: (x/1) / (1/x) = x^2 / 1
    RationalFunction<double> res_div2 = A / C;
    EXPECT_RF_EQ(res_div2, RationalFunction<double>{ Monomial<double>(1.0, x, 2) });

    // Division: (y/x) / (x/1) = y / x^2
    RationalFunction<double> res_div3 = D / A;
    Polynomial<double> num_div3 = Polynomial<double>(y);
    Polynomial<double> den_div3 = Polynomial<double>(Monomial<double>(1.0, x, 2));
    EXPECT_RF_EQ(res_div3, RationalFunction<double>{ num_div3, den_div3 });

    // Division by zero RF (Use default constructor for 0/1)
    RationalFunction<double> R0{}; // Represents 0/1
    EXPECT_THROW(A / R0, std::invalid_argument);
}

TEST(RationalFunctionTest, ArithmeticWithPolynomials) {
    // Use {} initializer
    RationalFunction<double> rf{ Polynomial<double>(x), Polynomial<double>(y) }; // x/y
    Polynomial<double> Pz(z);                                                    // z

    // RF + Poly: x/y + z = (x + yz) / y
    RationalFunction<double> res_add = rf + Pz;
    Polynomial<double> num_add = Polynomial<double>(x) + Polynomial<double>(y) * Pz;
    EXPECT_RF_EQ(res_add, RationalFunction<double>{ num_add, Polynomial<double>(y) });

    // Poly + RF: z + x/y = (zy + x) / y
    RationalFunction<double> res_add2 = Pz + rf;
    EXPECT_RF_EQ(res_add2, res_add);

    // Similar tests for -, *, /
    // RF * Poly: (x/y) * z = xz / y
    RationalFunction<double> res_mul = rf * Pz;
    Polynomial<double> num_mul = Polynomial<double>(x) * Pz;
    EXPECT_RF_EQ(res_mul, RationalFunction<double>{ num_mul, Polynomial<double>(y) });

    // Poly * RF: z * (x/y) = zx / y
    RationalFunction<double> res_mul2 = Pz * rf;
    EXPECT_RF_EQ(res_mul2, res_mul);

    // RF / Poly: (x/y) / z = x / (yz)
    RationalFunction<double> res_div = rf / Pz;
    Polynomial<double> den_div = Polynomial<double>(y) * Pz;
    EXPECT_RF_EQ(res_div, RationalFunction<double>{ Polynomial<double>(x), den_div });

    // Poly / RF: z / (x/y) = zy / x
    RationalFunction<double> res_div2 = Pz / rf;
    Polynomial<double> num_div2 = Pz * Polynomial<double>(y);
    EXPECT_RF_EQ(res_div2, RationalFunction<double>{ num_div2, Polynomial<double>(x) });
}

TEST(RationalFunctionTest, Evaluation) {
    // Use {} initializer
    RationalFunction<double> rf{ Polynomial<double>(x) + Polynomial<double>(Monomial<double>(1.0)),
                                 Polynomial<double>(y) }; // (x+1)/y
    std::map<Variable, double> values = { { x, 3.0 }, { y, 2.0 } };
    // (3+1)/2 = 4/2 = 2
    EXPECT_DOUBLE_EQ(rf.evaluate<double>(values), 2.0);

    // Denominator evaluates to zero
    std::map<Variable, double> values_y0 = { { x, 3.0 }, { y, 0.0 } };
    ...
    std::map<Variable, double> values_x_neg1 = { { x, -1.0 }, { y, 2.0 } };
    // (-1+1)/2 = 0/2 = 0
...
TEST(RationalFunctionTest, Differentiation) {
    // d(x/y)/dt = (x'y - xy') / y^2
    RationalFunction<double> rf_xy{ Polynomial<double>(x), Polynomial<double>(y) };
    RationalFunction<double> rf_diff = differentiate_wrt_t(rf_xy);
    ...
    Polynomial<double> Py_dot(y_dot);

    Polynomial<double> expected_num = Px_dot * Py - Px * Py_dot;
    Polynomial<double> expected_den = Py * Py; // y^2
    RationalFunction<double> expected_rf(expected_num, expected_den);

    EXPECT_RF_EQ(rf_diff, expected_rf);

    // d(k/x)/dt = (0*x - k*x') / x^2 = -kx' / x^2 (k is const)
    RationalFunction<double> rf_k_over_x{ Polynomial<double>(k), Polynomial<double>(x) };
    RationalFunction<double> rf_kox_diff = differentiate_wrt_t(rf_k_over_x);

    Polynomial<double> Pk(k);
    Polynomial<double> expected_kox_num = Polynomial<double>() - Pk * Px_dot; // -k*x'
    Polynomial<double> expected_kox_den = Px * Px;                            // x^2
    RationalFunction<double> expected_kox_rf(expected_kox_num, expected_kox_den);

    EXPECT_RF_EQ(rf_kox_diff, expected_kox_rf);

    // d(5.0)/dt = 0
    RationalFunction<double> rf_const{ Polynomial<double>(Monomial<double>(5.0)) };
    RationalFunction<double> rf_const_diff = differentiate_wrt_t(rf_const);
...
TEST(RationalFunctionTest, StreamOutput) {
    ...
    Polynomial<double> num = Polynomial<double>(x) + Polynomial<double>(Monomial<double>(1.0));
    Polynomial<double> den = Polynomial<double>(y) - Polynomial<double>(Monomial<double>(2.0));
    RationalFunction<double> rf_complex{ num, den };
    ss.str("");
    ss << rf_complex;
    ...
    // Zero: 0/1
    RationalFunction<double> rf_zero{}; // Use default constructor
    ss.str("");
    ss << rf_zero;
...
TEST(RationalFunctionTest, ComplexArithmetic) {
    RationalFunction<double> rx(x);
    RationalFunction<double> ry(y);
    RationalFunction<double> r_one(Polynomial<double>(Monomial<double>(1.0)));
    RationalFunction<double> r_two(Polynomial<double>(Monomial<double>(2.0)));

    // (x/1 + y/1) / (1/1 + 2/1) = (x+y)/3
    RationalFunction<double> res1 = (rx + ry) / (r_one + r_two);
    RationalFunction<double> exp1(Polynomial<double>(x) + Polynomial<double>(y),
                                  Polynomial<double>(Monomial<double>(3.0)));
    EXPECT_RF_EQ(res1, exp1);

    // (1 + 1/x) * (1 - 1/x) = 1 - 1/x^2 = (x^2 - 1) / x^2
    RationalFunction<double> r_one_over_x(r_one / rx);
    RationalFunction<double> res2 = (r_one + r_one_over_x) * (r_one - r_one_over_x);
    Polynomial<double> Px2_minus_1 =
      Polynomial<double>(Monomial<double>(1.0, x, 2)) - Polynomial<double>(Monomial<double>(1.0));
    Polynomial<double> Px2 = Polynomial<double>(Monomial<double>(1.0, x, 2));
    RationalFunction<double> exp2(Px2_minus_1, Px2);
...
TEST(RationalFunctionTest, DifferentiationWithDerivatives) {
    ...
    RationalFunction<double> rf_xd_over_x{ Polynomial<double>(x_dot), Polynomial<double>(x) };
    RationalFunction<double> res_diff = differentiate_wrt_t(rf_xd_over_x);

    Variable x_dotdot("x", 2);
    Polynomial<double> Px(x);
    Polynomial<double> Px_dot(x_dot);
    Polynomial<double> Px_dotdot(x_dotdot);

    Polynomial<double> exp_num = Px_dotdot * Px - Px_dot * Px_dot;
    Polynomial<double> exp_den = Px * Px;
    // Use {} initializer
    RationalFunction<double> exp_rf{ exp_num, exp_den };
...
TEST(RationalFunctionTest, EvaluationWithNaN) {
    ...
    RationalFunction<double> rx(x);
    RationalFunction<double> ry(y);
    RationalFunction<double> rf = (rx / ry) * (ry / rx); // Should simplify to 1/1 (if GCD worked)
                                                         // Without GCD: (xy) / (yx)

    std::map<Variable, double> values_x0 = { { x, 0.0 }, { y, 1.0 } };
